...
/**
 * FCA-style wrapper for meta-messenger.js
...
function buildFCAWrapper(client) {
    const threadsCache = new Map(); // threadID string -> { name, participantIDs?, adminIDs? }
    ...
    const api = {
        ...
        async getThreadInfo(threadID) {
            ...
            if (client.initialData && client.initialData.threads) {
                ...
                if (t) {
                    const info = { name: t.name || `Nhóm ${id}`, participantIDs: [], adminIDs: [] };
                    threadsCache.set(id, info);
                    return info;
                }
            }
            return { name: `Nhóm ${id}`, participantIDs: [], adminIDs: [] };
        },
